...
export default function PortfolioSection({
    ...
    const technologiesById = React.useMemo(() => {
        if (!technologies) return new Map<string, Technology>();
        return new Map(technologies.map(tech => [tech.id, tech]));
    }, [technologies]);
                ...
                <Dialog>
                    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        {projects.map((project) => {
                            ...
                            const projectTechs = project.technologyIds?.map(id => technologiesById.get(id)).filter(Boolean) as Technology[];
                                ...
                                <DialogTrigger asChild key={project.id} onClick={() => setSelectedProject(project)}>
                                    <Card className="flex flex-col overflow-hidden transition-shadow duration-300 hover:shadow-xl cursor-pointer rounded-3xl">
                                        ...
                                        {!hideDate && (
                                            ...
                                            <p className="mt-3 text-muted-foreground flex-grow line-clamp-2">{project.shortDescription}</p>
                                            {!hideTechs && projectTechs && projectTechs.length > 0 && (
              ...
              <DialogContent className="max-w-2xl" >
                ...
                <div className="flex flex-col space-y-6 ">
                    ...
                    <Carousel className="w-full">
                        <CarouselContent>
                            {(imagesByProjectId.get(selectedProject.id) || []).map((image, index) => (
