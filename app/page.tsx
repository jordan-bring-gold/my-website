...
export default function Home() {
  ...
  React.useEffect(() => {
    ...
    const uniqueSkillsArray = Array.from(allSkills);
    
    // Fisher-Yates shuffle algorithm
    for (let i = uniqueSkillsArray.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [uniqueSkillsArray[i], uniqueSkillsArray[j]] = [uniqueSkillsArray[j], uniqueSkillsArray[i]];
    }
    
    setShuffledSkills(uniqueSkillsArray);
